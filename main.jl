...
function komendy()
    while true
                ...
                if poprawonosc_komendy_szyfrowanie(dane_s)
                    ...
                    if dane_s.klucz_wpisywany != ""
                        if mod(length(dane_s.klucz_wpisywany),2) != 0
                            ...
                            dlugosc_klucza = length(dane_s.klucz_wpisywany)/2
                ...
                if poprawonosc_komendy_deszyfrowanie(dane_d)
                    ...
                    if dane_d.klucz_wpisywany != ""
                        if mod(length(dane_d.klucz_wpisywany),2) != 0
                            ...
                            dlugosc_klucza = length(dane_d.klucz_wpisywany)/2
        ...
        elseif komenda[1] == "?"
            ...
            print("szyfrowanie\n")
            print("   -p - podaj tekst\n")
            print("   -P - wczytaj tekst z pliku\n")
            print("   -k - podaj klucz\n")
            print("   -K - wczytaj klucz z pliku\n")
            print("   -s - zapisz klucz do pliku\n")
            print("   -S - zapisz szyfr do pliku\n")
            print("   -d - losowanie klucza z wykorzystaniem protokolu Diffie'go-Hellman'a\n")
            print("   -m - wybor trybu szyfrowania (CBC lub ECB)\n")
            print("   -g - losuj klucz (128, 192 lub 256 bitow)\n")
            print("deszyfrowanie\n")
            print("   -c - podaj szyfr\n")
            print("   -C - wczytaj szyfr z pliku\n")
            print("   -k - podaj klucz\n")
            print("   -K - wczytaj klucz z pliku\n")
            print("   -m - wybor trybu szyfrowania (CBC lub ECB)\n")
            print("koniec\n")
            print("wartosci kluczy nalezy wpisywac w HEX\n")
	    print("przykladowa komenda: szyfrowanie -p \"politechnika\" -K \"klucz.txt\" -m CBC\n")
        elseif komenda[1] == "md5"
            wartosc_md5 = ""
            wartosc_md5 = wartosc_tekst(wartosc_md5,komenda,2)
            if wartosc_md5 == ""
                print("Blad niepoprawna wartosc komendy md5\n")
            else 
                dlugosc_md5 = length(wartosc_md5)
                wektor_md5 = zeros(Int,dlugosc_md5)
                for i in 1:dlugosc_md5
                    wektor_md5[i] = codepoint(wartosc_md5[i])
                end
                wartosc_md5 = ""
                for i in 1:dlugosc_md5
                    znaki_md5 = string(wektor_md5[i],base=16)
                    if length(znaki_md5) == 1
                        znaki_md5 = string('0',znaki_md5)
                    end
                    wartosc_md5 = string(wartosc_md5,znaki_md5)
                end
                print(md5(wartosc_md5),'\n')
            end
        else
...
print("Autor: Mikolaj Gozdzielewski 193263\n")
print("Projekt wykorzystuje symetryczny szyfr blokowy AES (Advanced Encryption Standard), ktorego autorami sa Joan Daemen i Vincent Rijmen, funkcje skrotu MD5, ktora opracowal Ron Rivest oraz protokol Diffiego-Hellmana opracowany przez Witfielda Diffiego oraz Martina Hellmana.\n\n")
komendy()